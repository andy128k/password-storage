  ...
  (let ((data (app-data app)))
    (labels ((traverse (parent-iter)
               (iter (for i in-tree-model data children-of parent-iter)
                     (let ((entry (gtk:tree-model-value data i 0)))
                       (if (is-group entry)
                           (nunioning (traverse i) :test 'string=)
                           (adjoining (entry-username entry) :test 'string=))))))
  ...
  (let ((data (app-data app))
    ...
    (when iter
      (let ((entry (gtk:tree-store-value data iter 0)))
        (when (edit-entry entry (app-main-window app) "Edit" (get-usernames app))
          (update-row (app-data app) iter entry)
          (gtk:tree-model-filter-refilter (app-filter app))
...
(defun main ()
  ...
  (let* ((data (make-data))
    ...
    (let ((actions-common (app-actions-common app))
      ...
      (create-action actions-common (:name "new" :stock-id "gtk-new") "<Primary>n" (lambda-u (cb-new app)))
      (create-action actions-common (:name "open" :stock-id "gtk-open") "<Primary>o" (lambda-u (cb-open app)))
      (create-action actions-common (:name "merge-file" :label "_Merge file") nil (lambda-u (cb-merge-file app)))
      (create-action actions-common (:name "save" :stock-id "gtk-save") "<Primary>s" (lambda-u (cb-save app)))
      (create-action actions-common (:name "save-as" :stock-id "gtk-save-as") nil (lambda-u (cb-save-as app)))
      (create-action actions-common (:name "quit" :stock-id "gtk-quit") "<Primary>q" (lambda-u (e-close app)))

      (create-action actions-common (:name "edit-menu" :label "_Edit"))
      (create-action actions-common (:name "find" :label "_Find") "<Primary>f")
      (create-action actions-edit (:name "copy-name" :label "Copy _name" :sensitive nil) "<Primary>c" (lambda-u (cb-copy-name app)))
      (create-action actions-edit (:name "copy-password" :label "Copy pass_word" :sensitive nil) "<Primary><Shift>c" (lambda-u (cb-copy-password app)))
