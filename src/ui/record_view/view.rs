...
mod imp {
    ...
    impl ObjectImpl for PSRecordView {
        fn constructed(&self) {
            ...
            obj.grid_attach(&scrolled(&self.list_view)).set_row(0);
            obj.grid_attach(&self.search_bar).set_row(1);
    ...
    impl PSRecordView {
        ...
        async fn expand_path(&self, path: &TypedListStore<RecordNode>) -> bool {
            for record_node in path {
                let Some((_, row)) = self.find_record(&record_node) else {
                    return false;
                };
...
impl PSRecordView {
    ...
    pub async fn select_record(&self, record_node: &RecordNode) {
        if let Some((position, _)) = self.imp().find_record(record_node) {
