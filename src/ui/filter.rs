...
use glib::clone;
use gtk::prelude::*;
use gtk::{TreeIter, TreeModel, TreeModelFilter, TreeModelFilterExt};
...
fn record_satisfies(
    store: &PSStore,
    filter_func: &(dyn Fn(&Record) -> bool + 'static),
    iter: &TreeIter,
) -> bool {
...
impl PSTreeFilter {
    pub fn new() -> Self {
        let private = PSTreeFilterPrivate {
            model: None,
            filter: None,
            filter_func: None,
        };
        ...
        match model {
            Some(model) => {
                ...
                filter.set_visible_func(
                    clone!(@weak self as this => @default-return true, move |_model, iter| {
