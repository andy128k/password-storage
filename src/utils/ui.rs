...
struct BitsetForwardIterator<'bitset> {
...
pub fn bitset_iter(bitset: &gtk::Bitset) -> impl Iterator<Item = u32> + '_ {
    match gtk::BitsetIter::init_first(bitset) {
        Some((iter, _)) => BitsetForwardIterator { inner: Some(iter) },
        None => BitsetForwardIterator { inner: None },
    }
}

struct BitsetBackwardIterator<'bitset> {
    inner: Option<gtk::BitsetIter<'bitset>>,
}
...
pub fn bitset_iter_rev(bitset: &gtk::Bitset) -> impl Iterator<Item = u32> + '_ {
    match gtk::BitsetIter::init_last(bitset) {
